...
function getData(organ_id,Years_){
	...
	$(function() {
		...
		var y2_set = [];
		var y3_set = [];
		$.getJSON('http://localhost:8080/huntoms/station/displayPlanOnNetEY?organ_id='+organ_id+'&Years_='+Years_,
				function(data) {
					...
					showChart(titleName + Year_ + "年", xset, y1_set, y2_set,
							y3_set);
	...
	function createTable(station) {
		$('#table').empty();
		var th="<tr>"
				+ "<th>月份</th><th>计划上网电量(万度)</th><th>上网电量(万度)</th><th>上网计划完成率(%)</th>"
				+"</tr>";
		$('#table').append(th);
		...
		for(var i=0;i<len;i++){
			$('#table').append(		"<tr>"+
					...
					"<td>"+val(station[i].planOnE)+"</td>"+
					"<td>"+val(station[i].onNetE)+"</td>"+
					"<td>"+val(station[i].onNetE/station[i].planOnE*100)+"</td>"
					+"</tr>");
		}
	}
	function val(d){
