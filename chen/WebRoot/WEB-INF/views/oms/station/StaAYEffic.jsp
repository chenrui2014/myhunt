...
<script type="text/javascript">
	...
	function getData(organ_id) {
		$(function() {
			...
			var yTset = [];
			var yCset = [];
			$.getJSON('http://localhost:8080/huntoms/station/displayAYEffic?organ_id='+organ_id,
					function(data) {
						...
						showChart(titleName, xset, y1_set, y2_set);
	...
	function createTable(station) {
		$('#table').empty();
		var th = "<tr>"
				+ "<th>年份</th><th>理论发电量(万度)</th><th>发电量(万度)</th><th>发电效率(%)</th><th>单兆瓦发电量(万度)</th><th>等效利用小时(h)</th>"
				+ "</tr>";
		$('#table').append(th);
		...
		for ( var i = 0; i < len; i++) {
			$('#table').append(
					"<tr>" + "<td>" + station[i].year_ + "</td>" + "<td>"
							...
							+ "</td>" + "<td>"
							+ val(station[i].e / station[i].capacity) + "</td>"
							+ "<td>"
