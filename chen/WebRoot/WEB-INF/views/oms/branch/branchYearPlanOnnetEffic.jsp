...
function showHighCharts(orgName_,xAxis_,ylAxis_,yrAxis_){
	...
	function createTable(branch) {
		$('#table').empty();
		var th="<tr>"
				+ "<th>年份</th><th>计划上网电量（万度）</th><th>上网电量（万度）</th><th>计划完成率效率（%）</th>"
				+"</tr>";
		$('#table').append(th);
		...
		for(var i=0;i<len;i++){
			$('#table').append("<tr>"+
					"<td>"+branch[i].year+"</td>"+"<td>"+branch[i].planOne+"</td>"+"<td>"+branch[i].onnete+"</td>"+"<td>"+branch[i].planOnnetEffict+"%</td>"
					+"</tr>");
		}
	}
	
	function query(orgid){
		var url ='<%=request.getContextPath()%>/branch/dispayBranchYearPlanOnnetEffic?orgid='+orgid;
