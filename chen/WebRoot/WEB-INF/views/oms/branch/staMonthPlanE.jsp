...
function showHighCharts(orgName_,xAxis_,ylAxis_,yrAxis_){
	...
	function createTable(branch) {
		$('#table').empty();
		var th="<tr>"
				+ "<th>电站</th><th>计划发电量（万度）</th><th>发电量（万度）</th><th>计划完成率</th>"
				+"</tr>";
		$('#table').append(th);
		...
		for(var i=0;i<len;i++){
			$('#table').append("<tr>"+
					"<td>"+branch[i].organName+"</td>"+"<td>"+branch[i].planE+"</td>"+"<td>"+branch[i].e+"</td>"+"<td>"+branch[i].effict+"%</td>"
					+"</tr>");
		}
	}
	...
	function query(orgid,para){
		...
		var temp = para.split('-'); 
		var url = '<%=request.getContextPath()%>/branch/displayStaMonthPlanE?orgid='+orgid+'&parayear='+temp[0]+'&paramonth='+temp[1];
