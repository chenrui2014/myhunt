...
function showHighCharts(orgName_,xAxis_,ylAxis_,yrAxis_){
	...
	function createTable(branch) {
		$('#table').empty();
		var th="<tr>"
				+ "<th>日期</th><th>计划发电量（万度）</th><th>发电量（万度）</th><th>计划完成率（%）</th><th>单兆瓦发电量（万度）</th><th>等效利用小时（h）</th>"
				+"</tr>";
		$('#table').append(th);
		...
		for(var i=0;i<len;i++){
			$('#table').append("<tr>"+
					"<td>"+branch[i].year+"-"+branch[i].bmonth+"</td>"+"<td>"+branch[i].planE+"</td>"+"<td>"+branch[i].e+"</td>"+"<td>"+branch[i].planEffict+"%</td>"+"<td>"+branch[i].singlemwe+"</td>"+"<td>"+branch[i].eusehours+"</td>"
					+"</tr>");
		}
	}
	...
	function query(orgid,para){
		var  url ='<%=request.getContextPath()%>/branch/dispayBranchMonthPlanEffic?orgid='+orgid+'&parayear='+para;
