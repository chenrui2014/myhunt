...
function showHighCharts(orgName , xAxis_,ylAxis_,yrAxis_){
	...
	function createTable(branch) {
		...
		for(var i=0;i<len;i++){
			$('#table').append("<tr>"+
					"<td>"+branch[i].year+"</td>"+"<td>"+branch[i].e+"</td>"+"<td>"+branch[i].factorye+"</td>"+"<td>"+branch[i].purchasenete+"</td>"+"<td>"+branch[i].sumFactorye+"</td>"+"<td>"+branch[i].factoryEffict+"%</td>"+"<td>"+branch[i].sumFactoryEffict+"%</td>"
					+"</tr>");
		}
	}
...
function query(orgid){
	var url ='<%=request.getContextPath()%>/branch/displayBranchYearEnergyConsumption?orgid='+orgid;
