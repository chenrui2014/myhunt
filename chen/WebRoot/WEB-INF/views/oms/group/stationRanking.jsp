...
<head>
		...
		// 获取后台数据
		function getData(dateType,year,month,day) {
			if (dateType=='year') {
	            $.getJSON('http://localhost:8080/huntoms/group/getStationRankingYearData/'+year,function(data) {  
	            	var jsonObj = eval(data);
	            	show(jsonObj);   //显示图表
	            });  
			} else if (dateType=="month") {
	            $.getJSON('http://localhost:8080/huntoms/group/getStationRankingMonthData/'+year+'/'+month,function(data) {  
	            	var jsonObj = eval(data);
	            	show(jsonObj);   //显示图表
	            });  
			}
		...
		function search() {
			...
			for (var i=0;i<dateType.length;i++) {
				if (dateType[i].checked) {
					getData(dateType[i].value,document.getElementById('year').value,document.getElementById('month').value);
				}
			}
		}
