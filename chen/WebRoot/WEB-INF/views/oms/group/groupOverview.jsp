...
<head>
		...
		function getData(dateType,year,month,day) {
			if (dateType=='year') {
	            $.getJSON('http://localhost:8080/huntoms/group/getGroupOverviewYearData/'+year,function(data) {  
	            	var jsonObj = eval(data);
	            	show(jsonObj);   //显示图表
	            });  
			} else if (dateType=="month") {
	            $.getJSON('http://localhost:8080/huntoms/group/getGroupOverviewMonthData/'+year+'/'+month,function(data) {  
	            	var jsonObj = eval(data);
	            	show(jsonObj);   //显示图表
	            });  
			}
